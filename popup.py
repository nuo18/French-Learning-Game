...
class AddWords:
    def __init__(self, parent):
        ...
        self.clear_button = tk.Button(self.popup, text='Clear', font=('Arial', 18), bg='#28a745', fg='#ffffff', command=self.clear)
        self.clear_button.pack(padx=20, pady=20)
        
    # Commands
    def add_word(self):
        fre_word = self.fre_entry.get("1.0", "end-1c")
        eng_word = self.eng_entry.get("1.0", "end-1c")
        words.fre_words.append(fre_word)
        words.eng_words.append(eng_word)
    
    def clear(self):
        self.fre_entry.delete("1.0", "end")
        self.eng_entry.delete("1.0", "end")
